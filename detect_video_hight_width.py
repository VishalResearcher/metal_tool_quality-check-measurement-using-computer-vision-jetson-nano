...
def main(_argv):
    ...
    while True:
        ...
        for c_0 in contours:
            ...
            (xa,ya),(w,h),angle = rect
            box = cv2.boxPoints(rect)
            box = np.int0(box)
            print("-------------",box)
            epsilon = 0.01*cv2.arcLength(box, True)
            approx = cv2.approxPolyDP(box, epsilon, True)
            cv2.circle(frame,(int(xa),int(ya)),5,(0,0,255),-1)
            cv2.polylines(cropped_image, [box],True,(255,0,0),2)

        x_width = w/12.25
        h_hieght = h/12.35
        cv2.putText(cropped_image,"Hight-{}".format(round(x_width,1)),(int(xa),int(ya)),cv2.FONT_HERSHEY_PLAIN,1,(0,255,150),2)
        cv2.putText(cropped_image,"Width-{}".format(round(h_hieght,1)),(int(xa),int(ya-20)),cv2.FONT_HERSHEY_PLAIN,1,(255,255,0),2)
        ...
        if not FLAGS.dont_show:
            cv2.namedWindow("result_", cv2.WINDOW_NORMAL)
            cv2.imshow("result_", result)
